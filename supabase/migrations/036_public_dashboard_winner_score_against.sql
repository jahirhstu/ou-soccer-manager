...
as $$
  ...
  scored_team_rows as (
    select
      ...
      sum(sm.team_a_score)::integer goals_for,
      sum(sm.team_b_score)::integer goals_against
    ...
    select
      ...
      st.name team_name,
      captain.display_name captain_name,
      sum(sm.team_b_score)::integer goals_for,
      sum(sm.team_a_score)::integer goals_against
    from public.session_matches sm
  ...
  winner_summary as (
    select max(goals_for) winning_score
    from scored_teams
  ),
  latest_winner as (
    select
      ...
      ls.session_date,
      concat(max(st.goals_for), '-', max(st.goals_against)) score
    from scored_teams st
    cross join winner_summary ws
    join latest_session ls on ls.id = st.session_id
    where st.goals_for = ws.winning_score
    group by ws.winning_score, ls.id, ls.name, ls.session_date
  ),
