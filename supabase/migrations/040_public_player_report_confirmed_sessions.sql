...
language sql stable security definer set search_path = public as $$
  ...
  completed_player_sessions as (
    ...
    where s.status = 'completed'
      or s.session_date < (now() at time zone 'America/Toronto')::date
  ),
  completed_sessions as (
    ...
    from (
      select
        ...
        created_at,
        session_label,
        row_number() over (partition by player_id, season_id order by session_date desc, created_at desc) rn
      from completed_player_sessions
    ) ranked
    where ranked.rn <= 3
    group by ranked.player_id, ranked.season_id
  ),
  upcoming_player_sessions as (
    select
      a.player_id,
      s.season_id,
      s.session_date,
  ...
  upcoming_sessions as (
    ...
    from (
      select
        ...
        season_id,
        session_label,
        row_number() over (partition by player_id, season_id order by session_date asc, created_at asc) rn
      from upcoming_player_sessions
    ) ranked
    where ranked.rn = 1
  )
  select
