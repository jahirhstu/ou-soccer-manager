...
as $$
  ...
  scored_team_rows as (
    select
      ...
      st.name team_name,
      captain.display_name captain_name,
  ...
  scored_teams as (
    select
      ...
      team_name,
      captain_name,
      sum(goals_for)::integer goals_for,
      sum(goals_against)::integer goals_against,
      (sum(goals_for) - sum(goals_against))::integer goal_difference,
      sum(away_goals)::integer away_goals,
      sum(points)::integer points
    from scored_team_rows
    group by session_id, team_id, team_name, captain_name
  ),
  latest_winner_ranked as (
    select
      *,
      row_number() over (
        order by points desc, goal_difference desc, goals_for desc, away_goals desc, team_name
      ) winner_rank
    from scored_teams
  ),
  latest_winner as (
    select
      ...
      st.team_name,
      coalesce(st.captain_name, 'No captain') captain_name,
      st.points value,
      ls.id session_id,
      ls.name session_name,
      ls.session_date,
      concat(st.goals_for, '-', st.goals_against) score
    from latest_winner_ranked st
    join latest_session ls on ls.id = st.session_id
    where st.winner_rank = 1
  ),
