alter table public.session_matches
  add column if not exists away_team_id uuid references public.session_teams(id) on delete set null;

alter table public.session_matches
  drop constraint if exists session_matches_away_team_id_check;

alter table public.session_matches
  add constraint session_matches_away_team_id_check
  check (away_team_id is null or away_team_id = team_a_id or away_team_id = team_b_id);

create index if not exists session_matches_away_team_id_idx on public.session_matches(away_team_id);
...
begin
  ...
  loop
    ...
    insert into public.session_matches(session_id, match_number, team_a_id, team_b_id, away_team_id, team_a_score, team_b_score)
    values (p_session_id, saved_match_number, team_a_id, team_b_id, away_team_id, 0, 0)
...
drop function if exists public.public_leaderboards();
