...
export function FeatureScreen({ route, navigation }: NativeStackScreenProps<RootStackParamList, "Feature">) {
  ...
  return <FlatList
    ...
    renderItem={({ item }) => <Pressable disabled={route.params.featureKey !== "players" || profile?.role !== "admin"} onPress={() => navigation.navigate("PlayerEdit", { playerId: item.id })} style={styles.card}><Text style={styles.title}>{item.title}</Text>{item.subtitle ? <Text style={styles.subtitle}>{item.subtitle}</Text> : null}{item.detail ? <Text style={styles.detail}>{item.detail}</Text> : null}</Pressable>}
  />;
}
